...
/*   Updated: 2026/07/18 21:00:20 by hassmou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

size_t	count_va_env(const char *str, size_t *i, size_t *supp, t_shell *shell)
{
	size_t	cost_value;
	size_t	start_key;
	char	*key;
	...
	if (str[(*i) + 1] == '?')
		return (get_exit_status(i, supp, shell));
	...
	start_key = ((*i));
	start_key++;
	while ((ft_isalnum(str[start_key]) || (str[start_key] == '_')))
		start_key++;
...
size_t	get_exit_status(size_t *i, size_t *supp, t_shell *shell)
{
	size_t	cost_value;
	int	tmp;
